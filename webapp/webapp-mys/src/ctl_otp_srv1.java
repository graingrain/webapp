...
public class ctl_otp_srv1 {
    ...
    throws IOException, SQLException {
        ...
        //  String nomfichlire = repapache + "/conf.d/liste.cnf";
        String nomfichlire = repapache + "/conf.d/liste.cnf";
        ...
        System.out.println("-------------------------------------------------");
        System.out.println("ctl apache srv1 Luna  httpd.conf le " + vdatedeb);
        System.out.println(nomfichlire);
        System.out.println("------------------------------------v4-mysql-----");
        ...
        try {
            ...
            cnx = "jdbc:mysql://bd1.in.ac-reunion.fr/webapp";
            ...
            while((ligne = entree.readLine()) != null) {
                ...
                if(ligne.length() > 10 && ligne.indexOf("Listen")!=-1) {
                    System.out.println(red + "            ---------------------------------------"  + normal);
                    System.out.println(red + "                           " + ligne + normal);
                    System.out.println(red + "            ---------------------------------------"  + normal);
                    
                }
                if(ligne.length() > 10 && ligne.indexOf("ProxyPass")!=-1) {
                    System.out.println(yellow +  ligne + normal);
                ...
                if(ligne.length() > 15 && ligne.substring(0, 7).equals("Include") && ((ligne.indexOf("weblogic") != -1 ) ||(ligne.indexOf("ct-httpd") != -1 ) ) ) {
                    ...
                    do {
                        ...
                        if(ligne2.length() > 0 && ligne2.indexOf("<Location") != -1) {
                            ...
                            if(verbose3)      System.out.println(gray + "Location weblo=" + location + normal);
                        }
                        ...
                        if(ligne2.length() > 0 && !ligne2.substring(0, 1).equals("#") &&   ligne2.toLowerCase().indexOf("weblogicport") != -1) {
                            tokenizer = new StringTokenizer(ligne2, " , \t");
                            for(int t = 0; tokenizer.hasMoreTokens(); t++) {
                                String token = tokenizer.nextToken();
                                if(verbose3)
                            ...
                            etat = socket2( stmt_maj, type_host, host, port, "Weblogic   :" + location + "                ",location);
                        }
                        ...
                        if(ligne2.length() > 0 && !(ligne2.indexOf("#")!=-1)  && ligne2.indexOf("ProxyPass ") != -1) {
                            ...
                            for(int t = 0; tokenizer.hasMoreTokens(); t++) {
                                ...
                                if(t == 1) {
                                    ...
                                    etat = socket2( stmt_maj, type_host, host, port, "ProxyPass   :" + location + "                ",location);
                                }
                            }
                ...
                if(ligne.length() > 15 && ligne.substring(0, 7).equalsIgnoreCase("Include") && ligne.indexOf("tomcat") != -1) {
                    //   System.out.println( blue + "v2 "+"tomcat: " + ligne.substring(ligne.indexOf(" ") + 1, ligne.length()) + normal);
                    if(verbose2)
                        System.out.println( blue + "v2 "+"tomcat: " + ligne.substring(ligne.indexOf(" ") + 1, ligne.length()) + normal);
                    if(verbose2)
                        System.out.println( blue + "v2 " + "--------------------------------------------------------------------------" + normal);
                    String include =   ligne.substring(ligne.indexOf(" ") + 1, ligne.length());
                    String nom_include=repapache + "/" + include;
                    if (include.substring(0,1).equals("/")) nom_include=include;
                    BufferedReader entree_include = new BufferedReader(new FileReader(nom_include));
                    host = "";
                    port = 0;
                    ...
                    while((ligne2 = entree_include.readLine()) != null) {
                        if(verbose2)
                            System.out.println( blue + "v2 " + ligne2 + normal);
                        if(ligne2.length() > 13 && ligne2.substring(0, 12).equals("WebAppDeploy")) {
                            ...
                            if(host.length() > 0 && port > 0 && location.length() > 0) {
                                etat = socket2( stmt_maj, "webappD", host, port, "tomcat warp:" + location, location);
                                location = "";
                            }
                        }
                        ...
                        if(ligne2.length() > 6 && ligne2.indexOf("Mount") != -1) {
                            ...
                            etat = socket2( stmt_maj, "tomcat" , thost[trouve], tport[trouve], "tomcat(modjk)   :" + tloca[trouve], location);
                        }
                    }
                }
            }
    ...
    public static String socket2( Statement stmt_maj, String type_host, String host, int port, String type, String location)
    throws IOException {
        String reponse = "1";
        if(port == 0)
            return reponse;
        try {
            ...
            log(stmt_maj, vport, vtype, host, "1", location);
        }
            ...
            String vport = "" + port + "                                                    ";
            String vport1 = vport.substring(0, 10);
            String vappli = type.substring(0, type.indexOf("         "));
            System.out.println("vappli:"+vappli);
            String vtype = "(" + type.substring(0, type.indexOf("         ")) + ")" + "                                                                                                                               ";
            String affiche = blue + mess1 + red + vport1 + blue + vtype.substring(0, 50) + "                                                                                      ";
            System.out.println(affiche.substring(0, pos) + normal + normal + " [" + red + "  KO  " + normal + "]");
            reponse = "2";
            log(stmt_maj, vport, vtype, host, "2",location);
        }
        finally {
    ...
    public static String log(Statement stmt_maj, String port, String service, String host, String etat, String location) {
        String reponse = "1";
        try {
            Date DateJour = new Date();
            int month = DateJour.getMonth() + 1;
            ...
            if(code_domaine != 0)
                code_instancewa = bddaddIW(stmt_maj, code_domaine, port, location);
            if(verbose3)
    ...
    public static int bddaddportail(Statement stmt_maj, int machine, String type, String version, String repertoire, int port) {
        ...
        try {
            ...
            if(verbose3)
                System.out.println("sqladdbportail:" + select);
            stmt_maj.executeUpdate(select);
        }
        catch(SQLException ioe) {
            System.out.println("erreur sql:" + ioe);
    ...
    public static int bddaddIW(Statement stmt_maj, int machine, String port, String location) {
        int reponse = 0;
        try {
            Date DateJour = new Date();
            int month = DateJour.getMonth() + 1;
            ...
            if(nb == 0)
                select = "insert into instancewa (iw_do_code,iw_port1,iw_nom) values (" + machine + ",'" + port  + "','" + location + "')";
            else
    ...
    public static String gray = "\033[1;33m";
    //public static String blue = "\033[1;34m";
    public static String blue = "\033[1;33m";
